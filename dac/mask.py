...
def get_mask(attribution, real_img, fake_img, real_class, fake_class, 
             net_module, checkpoint_path, input_shape, input_nc=3, output_classes=5,
    ...
    real_img = real_img.transpose(2,1,0)
    fake_img = fake_img.transpose(2,1,0)
    ...
    for k in range(0,steps+1):
        ...
        real_img2 = real_img.transpose(2,1,0)
        fake_img2 = fake_img.transpose(2,1,0)
        #print("fake img shape", fake_img2.shape)
        fake_img_norm = normalize_image(copy.deepcopy(fake_img2))
        #print("fake img norm shape", fake_img_norm.shape)
        out_fake = run_inference(net, fake_img_norm)
        
        real_img_norm = normalize_image(copy.deepcopy(real_img2))

        out_real = run_inference(net, real_img_norm)

        im_copied_norm = normalize_image(copy.deepcopy(copyto))
        out_copyto = run_inference(net, im_copied_norm.transpose(2,1,0))
